...
class ExpData:
    ...
    def __init__( self, exp_vars:list, data:dict, configs:dict={} ):
        ...
        self.__data = data
        check_list = [self.__check_exp_vars(exp_vars), self.__check_shape()]
    ...
    def __check_exp_vars( self, exp_vars:list )->int:
        """
        If all the linked exp var have same length, return true, else return false
        """
        checked_exp_vars = []
        for ev in exp_vars:
            input_name = ev[0]
        ...
        self.__exp_vars = checked_exp_vars
        return True
